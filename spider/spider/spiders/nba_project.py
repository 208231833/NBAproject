...
class NbaProjectSpider(scrapy.Spider):
    name = 'nba_project'
    ...
    def parse3(self,response):
        ...
        for i in range(3, 33):
            team=team_data()
            team['rank']= response.xpath("//tbody/tr[{}]/td[1]/text()".format(i)).extract()[0]
            team['team_name'] = response.xpath("//tbody/tr[{}]/td[2]/a/text()".format(i)).extract()[0]
            url = response.xpath("//tbody/tr[{}]/td[2]/a/@href".format(i)).extract()[0]
            team['hit_percentage'] = response.xpath("//tbody/tr[{}]/td[3]/text()".format(i)).extract()[0]
            team['hit_number']= response.xpath("//tbody/tr[{}]/td[4]/text()".format(i)).extract()[0]
            team['out_number']=response.xpath("//tbody/tr[{}]/td[5]/text()".format(i)).extract()[0]
            team['three_percentage']=response.xpath("//tbody/tr[{}]/td[6]/text()".format(i)).extract()[0]
            team['three_number']=response.xpath("//tbody/tr[{}]/td[7]/text()".format(i)).extract()[0]
            team['three_out_number']=response.xpath("//tbody/tr[{}]/td[8]/text()".format(i)).extract()[0]
            team['free_percentage']=response.xpath("//tbody/tr[{}]/td[9]/text()".format(i)).extract()[0]
            team['free_number']=response.xpath("//tbody/tr[{}]/td[10]/text()".format(i)).extract()[0]
            team['free_out_number']=response.xpath("//tbody/tr[{}]/td[11]/text()".format(i)).extract()[0]
            team['All_rebound']=response.xpath("//tbody/tr[{}]/td[12]/text()".format(i)).extract()[0]
            team['fight_rebound']=response.xpath("//tbody/tr[{}]/td[13]/text()".format(i)).extract()[0]
            team['defind_rebound']=response.xpath("//tbody/tr[{}]/td[14]/text()".format(i)).extract()[0]
            team['assist']=response.xpath("//tbody/tr[{}]/td[15]/text()".format(i)).extract()[0]
            team['fault']=response.xpath("//tbody/tr[{}]/td[16]/text()".format(i)).extract()[0]
            team['steal']=response.xpath("//tbody/tr[{}]/td[17]/text()".format(i)).extract()[0]
            team['block']=response.xpath("//tbody/tr[{}]/td[18]/text()".format(i)).extract()[0]
            team['foul']=response.xpath("//tbody/tr[{}]/td[19]/text()".format(i)).extract()[0]
            team['scores']=response.xpath("//tbody/tr[{}]/td[20]/text()".format(i)).extract()[0]
            print(team)
            yield team
            yield scrapy.Request(url=url,callback=self.parse4)
    def parse4(self,response):
        team_info_temp=team_info()
        team_info_temp['name']=response.xpath("//div[@class='team_data']/h2/span[@class='title-text']/text()").extract()[0]
        team_info_temp['url']=response.xpath("//div[@class='img']/img/@src").extract()[0]
        team_info_temp['enter_time']=response.xpath("//div[@class='font']/p[1]/text()").extract()[0].split("：")[1]
        team_info_temp['home']=response.xpath("//div[@class='font']/p[2]/text()").extract()[0].split("：")[1]
        team_info_temp['web']=response.xpath("//div[@class='font']/p[3]/a/@href").extract()[0]
        team_info_temp['coach']=response.xpath("//div[@class='font']/p[4]/text()").extract()[0].split("：")[1]
        team_info_temp['describes']=response.xpath("//div[@class='txt']/text()").extract()[0].strip()
        yield team_info_temp
